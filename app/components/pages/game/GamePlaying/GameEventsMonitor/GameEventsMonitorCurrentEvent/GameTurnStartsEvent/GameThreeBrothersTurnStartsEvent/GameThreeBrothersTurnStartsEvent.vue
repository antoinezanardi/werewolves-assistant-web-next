<template>
  <GameEventWithTexts
    id="game-three-brothers-turn-starts-event"
...
const gameEventTexts = computed<string[]>(() => {
  if (isFirstNight.value) {
    return [t("components.GameThreeBrothersTurnStartsEvent.threeBrothersMeetEachOtherForFirstTime")];
  }
  return [t("components.GameThreeBrothersTurnStartsEvent.threeBrothersMeetEachOther")];
});

playSoundEffect("boys-playing");
</script>