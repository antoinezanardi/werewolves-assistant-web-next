...
Feature: ⚙️ Game Lobby Options Hub
  ...
  Scenario: ⚙️ Options Hub is displayed when user clicks game options button in lobby
    ...
    And the page should match the snapshot with name "Game Lobby Options Hub on Roles tab"

    When the user clicks on the tab with name "Composition"
    And the heading with name "Composition is hidden" should be visible
    Then the page should match the snapshot with name "Game Lobby Options Hub on Composition tab"

    When the user clicks on the tab with name "Votes"
    And the heading with name "Votes can be skipped" should be visible
    Then the page should match the snapshot with name "Game Lobby Options Hub on Votes tab"
